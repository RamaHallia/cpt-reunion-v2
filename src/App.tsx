...
function App() {
  ...
  const [view, setView] = useState<'landing' | 'auth' | 'record' | 'history' | 'detail' | 'settings' | 'upload' | 'dashboard' | 'gmail-callback'>(getInitialView());
  ...
  const handleStartRecording = async () => {
    // Vérifier le quota avant de démarrer
    try {
      ...
      if (subscription && subscription.plan_type === 'starter') {
        ...
        if (usagePercent > 90) {
          ...
          const shouldContinue = confirm(`⚠️ Attention : Il vous reste seulement ${remaining} minutes ce mois-ci.\n\nVoulez-vous continuer l'enregistrement ?`);
          if (!shouldContinue) {
            return;
          }
        }
  ...
  const handleViewMeetingById = async (meetingId: string) => {
    // Charger la réunion depuis la DB si pas déjà dans la liste
    const existingMeeting = meetings.find(m => m.id === meetingId);
    
    if (existingMeeting) {
      handleViewMeeting(existingMeeting);
    } else {
      // Charger la réunion depuis la DB
      try {
        const { data, error } = await supabase
          .from('meetings')
          .select('*')
        ...
        <div className={view === 'record' ? 'flex gap-6 h-full' : 'max-w-6xl mx-auto px-4 md:px-8 py-4 md:py-8 min-h-screen'}>
          {view === 'record' ? (
            ...
            <AudioUpload
              userId={user?.id || ''}
              onSuccess={(meetingId) => {
                loadMeetings(true); // Force reload après upload
                if (meetingId) {
